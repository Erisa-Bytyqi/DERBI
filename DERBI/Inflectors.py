...
from compound_split import char_split as splitter
import spacy
import re
import json
import os
...
class BasicInflector:
    ...
    def __call__(self, token: spacy.tokens.token.Token, target_tags: str) -> str:
        # the common way:
        # 1. search in lexicon
        output, remaining_tags = self.search_in_lexicon(
            token.lemma_.lower(), target_tags)
...
class AUXInflector(BasicInflector):
    ...
    def __call__(self, token: spacy.tokens.token.Token, target_tags: str):
        ...
        if 'Verbform=Part' in target_tags:
            target_tags = re.sub(
                'Verbform=Part', 'Tense=Past|Verbform=Part', target_tags)
...
class DETInflector(BasicInflector):
    ...
    def __call__(self, token: spacy.tokens.token.Token, target_tags: str) -> str:
        ...
        input = token.lemma_.lower(
        ) if 'Poss=Yes' not in target_tags else self.parse_poss_dets(token.text.lower())
    ...
    def __call__(self, token: spacy.tokens.token.Token, target_tags: str) -> str:
        ...
        if 'Declination=' in target_tags:
            if re.search('e[mnrs]{0,1}$', token.norm_) is None:
                raise ValueError('Could not decline word "' +
                                 token.norm_ + '" as an ADJ.')
...
class VERBInflector(AUXInflector):
    ...
    def sep_prefixes(self, token: str) -> str:
        ...
        def syls(x): return len(re.findall('[aeiouyäöüEIUY]', x))

        prefs = []
        # detect and separate prefixes
        prefixes_pattern = re.compile(
            '^' + '|^'.join(self.prefixes['sep'] + self.prefixes['insep']))
        ...
        # restore diphthongs
        def restore_dis(x, matches): return x if not len(matches) else restore_dis(re.sub(matches[-1],
                                                                                          {v: k for k, v in dis.items()}[
            matches[-1]], x),
            matches[:-1])
        # we also need to know if the prefix complex is separable or inseparable
        insep = ((prefs[0] in self.prefixes['insep'])
                 or (prefs[-1] in self.prefixes['insep']))
        prefs = restore_dis(''.join(prefs), re.findall(
            '|'.join({v: k for k, v in dis.items()}.keys()), ''.join(prefs)))
